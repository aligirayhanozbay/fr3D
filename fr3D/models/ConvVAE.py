...
def sampling(z_mean: tf.Tensor, z_log_sigma: tf.Tensor):
    epsilon = tf.random.normal(tf.shape(z_log_sigma), mean=0.0, stddev=1.0)
    return z_mean + tf.exp(z_log_sigma/2)*epsilon

@tf.function
def VAELoss(z_mean: tf.Tensor, z_log_sigma: tf.Tensor):
    losses_per_sample = -0.5*tf.reduce_sum(1+z_log_sigma-z_mean**2-tf.exp(z_log_sigma), axis=-1)
    return tf.reduce_mean(losses_per_sample, axis=0)

class ConvVAE(ConvAutoencoder):
    def __init__(self, *args, bottleneck_size=None, **kwargs):
        super().__init__(*args, auto_build=False, **kwargs)

        self.latent_sampler = self.create_latent_sampler(self.encoder.output_shape[1:], bottleneck_size)

        self.build(self.encoder.input_shape)
        

    @staticmethod
    def create_latent_sampler(latent_space_shape, bottleneck_size=None):
        ...
        dense_z_log_sigma = tf.keras.layers.Dense(bottleneck_size, name='z_log_sigma')(x)
